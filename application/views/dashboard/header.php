...
<head>
    ...
    <!-- Begin colorpicker -->
    <script src="<?=$base?>js/dashboard/colorpicker.js"></script>
    <script type="text/javascript" src="<?=$base?>js/dashboard/eye.js"></script>
    <script type="text/javascript" src="<?=$base?>js/dashboard/utils.js"></script>
    <script type="text/javascript" src="<?=$base?>js/dashboard/layout.js?ver=1.0.2"></script>
        ...
        $( document ).ready(function() {
            ...
            $('.popup-help').click(function(e){
                ...
                $.post( "<?=$base?>dashboardajax/getpopupcontent", {popupid: $(this).attr("data-id"), type: $(this).attr("data-type")})
                  .done(function( data ) {
                      data = eval('(' + data + ')')
                      $("#help-title").html( data['Descript'].replace('0', '') );
                      $("#help-content").html( '<p>'+data['HelpContent'].replace('0', '')+'</p>' );
                  });
            });
            $('.popup-help').fancybox();
            <? $popup = _getPopup( (isset($pageId) ? $pageId : '')); if( !empty($popup) && $popup['ID'] != '3' ): ?>
                $('.forced-popup').click();
            <? endif; ?>
        }, 'json');
    </script>
...
<body>
            ...
            <div id="popup" style="display:none;">
                <div class="lightbox-content hidden-xs">
                    ...
                    <div class="lightbox-mid">
                        ...
                        <? elseif( $popup['Type'] == '3' ): ?>
                            <form action="<?=$base?>dashboard/newevent" method="post" id="post-facebook-event-form">
        ...
        <ul class="clearfix " id="menu">
            ...
            <li><a href="#" class="<? if(isset($active_menu) && $active_menu == 'events'):?>active<? endif; ?> icon-events"><i></i>Events</a></li>
...
<div id="top" class="hidden-xs">
  ...
  <a href="<?=$base?>dashboard/newevent" class="button icon icon-plus hidden-xs">Create event</a>
  <span id="menu-start" class="hidden-xs" >MENU</span>
  <ul class="clearfix " id="menu">
    <li><a href="<?=$base?>dashboard" class="<? if(!isset($active_menu)):?>active<? endif; ?> icon-grid"><i></i>Dashboard</a></li>
    <li><a href="<?=$base?>dashboard/events" class="<? if(isset($active_menu) && $active_menu == 'events'):?>active<? endif; ?> icon-events"><i></i>Events</a></li>
        <!--<li><a href="#" class="icon-reports"><i></i>Reports</a></li>
        <li><a href="#" class="icon-stats"><i></i>Statistics</a></li>
        <li><a href="#" class="icon-inbox"><i></i>Messages <span class="dark pill">22</span></a></li>-->
    </ul>
...
<div id="wrapper">
  ...
  <div id="options">
    ...
    <a href="<?=$base?>dashboard/settings" class="settings">Profile</a>
