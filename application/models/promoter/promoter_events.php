...
class Promoter_events extends CI_Model
{
	/**
	 *	Object properties
	 */
	...
	// Load all the events
	public function load_events($limit=999999999)
	{
		// Query to get all the events from a certain user that's logged in.
		$query = "SELECT * FROM events WHERE UserID=:user_id AND Status != '2' LIMIT ".$limit;
		...
		{
			$events[$key]['preferences'] = $this->getPreferences($event['EventID']);
			$events[$key]['eventscustomisationpreferences'] = $this->getCustomisationPreferences($event['EventID']);

			$events[$key]['eventsponsorpreferences'] = $this->getEventSponsorPreferences($event['EventID']);
			...
			$events[$key]['current_status'] = $this->getStatus($events[$key]);
		}
		return $events;
	}
	/*
		Get the status of an event if all the fields have been filled in to go public
	 */
	public function getStatus($event){
		//echo '<xmp>'; print_r($event);echo '</xmp>'; exit();
		if(
			!isset($event['preferences']['description']) || $event['preferences']['description'] == '' ||
			!isset($event['preferences']['eventname']) || $event['preferences']['eventname'] == '' ||
			!isset($event['preferences']['start_date']) || $event['preferences']['start_date'] == '' ||
			!isset($event['preferences']['start_time']) || $event['preferences']['start_time'] == '' ||
			!isset($event['preferences']['address']) || $event['preferences']['address'] == ''
			){
			return array('url' => ''.$event['EventID'], 'message' => 'Missing primary information');
		}
		elseif(
			empty($event['tickets'])
			){
			return array('url' => ''.$event['EventID'].'/tickets', 'message' => 'Missing tickets');
		}
		else{
			return array('message' => 'Good to go!');
		}
	}
	/*
	...
	public function load_event($event_id){
		...
		$event['preferences'] = $this->getPreferences($event_id);
		$event['eventscustomisationpreferences'] = $this->getCustomisationPreferences($event_id);
		$event['eventsponsorpreferences'] = $this->getEventSponsorPreferences($event_id);
	...
	/*
		Functions to get preferences by event ID
	 */
	public function getPreferences($event_id){
		$preferences = array();
	...
	public function getCustomisationPreferences($event_id){
		$preferences = array();
	...
	public function getEventSponsorPreferences($event_id){
		$preferences = array();
		...
		while ($row = $db->fetch(PDO::FETCH_ASSOC)){
			...
			while($row2 = $stmt->fetch(PDO::FETCH_ASSOC)){
				$preferences[$row['Field']]['preferences'][$row2['Field']] = $row2;
			}
		}
		return $preferences;
	}
	...
	************************************/
	...
	public function registerNewEvent(){
		try{
			if(isset($_POST['event_id']) && $_POST['event_id'] != ''){
				$query = "UPDATE events SET EventName = :value WHERE EventID = :id";
				$db = $this->database->prepare($query);
				$db->bindValue(":id", $_POST['event_id']);
				$db->bindValue(":value", $_POST['eventname']);
				$db->execute();
	...
	public function registerCustomisationVariables(){
		try{
			if(isset($_FILES['upload-header']) && $_FILES['upload-header'] != ''){
				$extension = strtolower(strrchr($_FILES["upload-header"]["name"], '.'));
				if($extension != '.jpg' && $extension != '.jpeg' && $extension != '.gif' && $extension != '.png'){
					...
					if ($_FILES["upload-header"]["error"] == UPLOAD_ERR_OK) {
						...
						$result = $this->resize->openImage("./uploads/cover_photos/_src/".$_POST['header_photo']);
	...
	public function deleteEvent($event_id){
		$query = "UPDATE events SET Status = :value WHERE EventID = :id";
		$db = $this->database->prepare($query);
		$db->bindValue(":id", $event_id);
		$db->bindValue(":value", 2);
		$db->execute();
		return true;
	}
}
